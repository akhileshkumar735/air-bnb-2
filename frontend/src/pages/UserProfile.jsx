...
export default function UserProfile() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || t("profile.fail_update"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="glass-panel border border-slate-200/50 dark:border-slate-800/40 shadow-2xl rounded-3xl p-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-center text-center sm:text-left space-y-4 sm:space-y-0 sm:space-x-6 pb-6 border-b border-slate-200/50 dark:border-slate-800/35">
          <div className="relative group">
            ...
            {editing && (
              <button 
                ...
                title={t("profile.randomize_avatar")}
              >
          ...
          <div className="text-center sm:text-left space-y-1.5 flex-grow">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">
              {user.role === "admin" ? t("profile.system_admin") : t("profile.member")}
            </p>
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mt-2">
              <div className="flex items-center text-[10px] text-brand dark:text-brand-light font-extrabold uppercase bg-brand/10 dark:bg-brand/15 px-2.5 py-0.5 rounded-lg tracking-wider">
                <ShieldCheck className="h-3.5 w-3.5 mr-1.5 shrink-0" /> {t("profile.verified")}
              </div>
              {user.role === "admin" && (
                <div className="flex items-center text-[10px] text-indigo-600 dark:text-indigo-400 font-extrabold uppercase bg-indigo-50 dark:bg-indigo-955/40 px-2.5 py-0.5 rounded-lg tracking-wider border border-indigo-200/30">
                  🛡️ {t("profile.system_admin")}
                </div>
              )}
            </div>
          </div>
        ...
        <form onSubmit={handleSave} className="space-y-5">
          <div className="flex flex-col space-y-1.5 text-left">
            <label className="text-[10px] font-extrabold text-slate-455 dark:text-slate-500 uppercase tracking-wider flex items-center">
              <User className="h-3.5 w-3.5 mr-2 text-brand" /> {t("profile.username")}
            </label>
            <input 
              type="text" 
              required
          ...
          <div className="flex flex-col space-y-1.5 text-left">
            <label className="text-[10px] font-extrabold text-slate-455 dark:text-slate-500 uppercase tracking-wider flex items-center">
              <Mail className="h-3.5 w-3.5 mr-2 text-brand" /> {t("profile.email")}
            </label>
            <input 
              type="email" 
              disabled
            ...
            />
            <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider">{t("profile.email_locked")}</p>
          </div>

          {editing && (
            <div className="flex flex-col space-y-1.5 text-left">
              <label className="text-[10px] font-extrabold text-slate-455 dark:text-slate-500 uppercase tracking-wider">{t("profile.avatar_url")}</label>
              <input 
                type="text" 
                value={avatar}
            ...
            {editing ? (
              <div className="flex space-x-2.5">
                <button
                  ...
                  className="px-5 py-2.5 border border-slate-200 dark:border-slate-800 text-slate-660 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-full text-xs font-bold transition active:scale-95 cursor-pointer"
                >
                  {t("profile.cancel")}
                </button>
                <button
